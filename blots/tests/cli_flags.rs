...
fn test_evaluate_flag() {
    ...
    let (stdout, _, success) = run_blots(&["-e"], Some("output result = 42"));
...
fn test_inputs_flag_with_file() {
    ...
    writeln!(file, "output x = inputs.x ?? 0\noutput y = inputs.y ?? 0").unwrap();
...
fn test_output_flag() {
    ...
    writeln!(source_file, "output result = 42\noutput message = \"test\"").unwrap();
...
fn test_combined_flags() {
    // Create a test file
    let mut source_file = NamedTempFile::new().unwrap();
    writeln!(source_file, "output result = inputs.x * 2").unwrap();
...
fn test_invalid_json_stdin() {
    ...
    let (_, stderr, success) = run_blots(&[file.path().to_str().unwrap()], Some("not valid json"));
...
fn test_empty_outputs() {
    ...
    let (stdout, _, success) = run_blots(&[file.path().to_str().unwrap()], None);
...
fn test_output_with_nullish_coalescing() {
    ...
    let (stdout, _, success) =
        run_blots(&[file.path().to_str().unwrap(), "-i", r#"{"x": 10}"#], None);
