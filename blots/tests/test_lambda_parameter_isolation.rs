...
fn test_lambda_parameter_not_substituted_simple() {
    ...
    assert_eq!(
        func_src, "(x) => x + 1",
        "Lambda parameter should not be substituted with outer scope value"
    );
    assert!(
        !func_src.contains("42"),
        "Lambda body should not contain the value '42'"
    );
}
...
fn test_lambda_multiple_parameters_not_substituted() {
    ...
    assert_eq!(
        func_src, "(a, b) => a + b",
        "Lambda parameters should not be substituted"
    );
    assert!(
        !func_src.contains("10"),
        "Lambda body should not contain value '10'"
    );
    assert!(
        !func_src.contains("20"),
        "Lambda body should not contain value '20'"
    );
}
...
fn test_partial_application_captures_value_preserves_parameter() {
    ...
    assert!(
        !func_src.contains("x"),
        "Captured variable 'x' should be substituted with value"
    );
...
fn test_nested_lambdas_preserve_parameters() {
    ...
    assert_eq!(func_src, "(x) => (y) => x + y");
    assert!(
        !func_src.contains("100"),
        "Outer scope value should not pollute lambda"
    );
}
...
fn test_lambda_in_conditional_preserves_parameters() {
    ...
    assert!(
        !func_src.contains("42"),
        "Lambda body should not contain outer scope value"
    );
}
...
fn test_higher_order_function_composition() {
    ...
    assert!(
        !func_src.contains("99"),
        "Lambda should not capture outer scope value with same name as parameter"
    );
}
...
fn test_multiple_lambdas_same_parameter_name() {
    ...
    assert!(
        !func_src.contains("1000"),
        "Parameter 'x' should not be replaced with outer value"
    );
}
...
fn test_lambda_with_list_operations_preserves_parameters() {
    ...
    assert!(
        func_src.contains("x"),
        "Outer parameter 'x' should be preserved"
    );
    assert!(
        func_src.contains("y"),
        "Inner parameter 'y' should be preserved"
    );
    assert!(
        !func_src.contains("50"),
        "Should not contain outer scope value"
    );
}
